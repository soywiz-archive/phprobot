...
int strstrcase(const char *s1, const char *s2) {
	char c, c2, ret;
	int n, n2, n3, l1 = strlen(s1), l2 = strlen(s2), l3;
	if (l2 == 0) return -1;
	if (l1 > l2) return -1;
	l3 = l2 - l1;
	c2 = s2[0];
...
/*
__inline char *rindex(const char *str, int fnd) {
	return strrchr(str, fnd);	
}
*/
...
static void hashinit(void) {
	int lop;
	for (lop = 0; lop < 256; lop++) filelist_hash[lop] = -1;
}
...
FILELIST *filelist_find(char *fname) {
	...
	for (hash = filelist_hash[filehash(fname)]; hash >= 0; hash = filelist[hash].next) {
		if (strcasecmp(filelist[hash].fn,fname) == 0) break;
	}
 ...
 *------------------------------------------
 */
DLL_EXPORT int grfio_size(char *fname) {
	FILELIST *entry;
 ...
 *------------------------------------------
 */
DLL_EXPORT void* grfio_reads(char *fname, int *size) {
	FILE *in = NULL;
...
static int grfio_entryread(char *gfname, int gentry) {
	...
	if ((fp = fopen(gfname, "rb")) == NULL) return 1;
...
DLL_EXPORT int grfio_add(char *fname) {
	int len, result;
	char *buf;
...
int main() {
	...
	grfio_init();
	grfio_add("e:\\juegos\\ragnarok\\data.grf");

	grfio_resourcecheck();

	printf("LEN: %i", grfio_size("data\\resnametable.txt"));

	//test = filelist_find("grf");
...
/*
DLL_EXPORT int grfio_find(char *name) {
	int hash;
